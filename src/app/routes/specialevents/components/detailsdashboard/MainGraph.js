...
import {
  graphLegendItemColor,
  graphParseSeriesNames,
  getDateFormat,
  emptyData,
} from '../../../../helpers/HighChartHelper';
...
export default class MainGraph extends React.Component {
  ...
  componentDidMount() {
    this.setState({
      data: emptyData,
      options: this.getOptions('spline'),
    });
  }
  ...
  setData = (filter) => {
    ...
    getSpecialEventChart(filterWithGraphId)
      .then((responseData) => {
        const { data, alias } = responseData;
        this.setState(
          {
            ...responseData,
            data: graphParseSeriesNames(data, alias),
            options: this.getOptions('spline', responseData.groupDateTime, responseData.titles),
          },
          this.finishLoading
        );
      })
  ...
  getOptions = (graphType = 'spline', groupDateTime = '24h', titles = {}) => {
  ...
  render() {
    const { data, options, titles = {}, loading } = this.state;

...
export default class ResultTypesForm extends React.Component {
  constructor(props) {
    super(props);
  ...
  submit = (event) => {
    ...
    devicesApi
      ...
      .then(() => {
        successMessage('Success', `Sensor successfully ${data.id ? 'edited' : 'created'}`, 5000);
        ...
        <form className="smart-form" onSubmit={this.submit}>
          <fieldset>
            <label className="label" style={{ fontSize: '1.7rem', padding: '10px' }}>
              Result Type Edit ID: {this.state.id}
            </label>
